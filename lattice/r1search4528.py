#!/usr/bin/python3
#
# Fitness search on central channel  radius (r1)
#
...
class R1Search(object):
    'Find k(CR) relation for different inner diameters'
    def __init__(self, stretch_geometry:int=1, search_path:str='./.runs/'):
        ...
        self.min:float  =  0.5  # Minimum r1 [cm]
        self.max:float  =  6.0  # Maximum r1 [cm]
        self.N:int      =   23  # Number of steps in r1
    ...
    def prepare_searches(self):
        ...
        for my_r1 in self.steps:
            s   = SlitCritSearch()
            s.r1 = my_r1          # Define geometry for slit search
            if self.stretch_geometry == 2:  # Scale all
                s.l  = self.l  * my_r1/self.r1
                s.r2 = self.r2 * my_r1/self.r1
                s.r3 = self.r3 * my_r1/self.r1
                s.l4 = self.l4 * my_r1/self.r1
...
#                    print("[DBGr1] 1 ", s.l, s.r1, s.r2, s.r3, s.l4)
            if self.stretch_geometry == 0:  # Keep l and r2, scale r3 and l4
                ...
                s.l4 = nominal_l4(s.l)
#                if my_debug:
 #                   print("[DBGr1] 0 ", s.l, s.r1, s.r2, s.r3, s.l4)
            if not validate_geometry(s.l, s.r1, s.r2, s.r3, s.l4): # Make sure the lattice dimensions are consistent
                if my_debug:
                    print("[DBGr1] geometry problem, skipping ", s.l, s.r1, s.r2, s.r3, s.l4)
                r1_to_remove  = np.array([my_r1])   # Remove my_r1 from the steps
                self.steps = np.setdiff1d(self.steps, r1_to_remove)
                continue
    ...
    def plot_searches(self):
        ...
        for s in self.critsearches:    # Make plots
            s.plot("fit_r1_%d_%08.5f.png" % (self.stretch_geometry, s.r1))
...
#            print("[#####]", self.ks.shape, len(my_ks))
    ...
    def plot_ks_at_cr(self, cr:float=0.97, plot_file:str='k_vs_r1.pdf'):
        'Plot k|cr vs r1'
        if self.steps.size == 0:
            print("[ERROR] Search not ready, run prepare_searches()")
            return False
...
if __name__ == '__main__':
    print("This is a module to run r1 parameter search on ORNL-4528 lattice.")
    input("Press Ctrl+C to quit, or enter else to test it. ")
    stretch_geometry = 2            # 2, 1, 0
    r = R1Search(stretch_geometry)  # Call constructor
    ...
    r.plot_ks_at_cr(0.97,"./plots/r1_%d_k_at_cr097pct.png" % stretch_geometry)
    r.plot_ks_at_cr(0.95,"./plots/r1_%d_k_at_cr095pct.png" % stretch_geometry)
    r.plot_ks_at_cr(0.98,"./plots/r1_%d_k_at_cr098pct.png" % stretch_geometry)
    r.plot_ks_at_cr(1.00,"./plots/r1_%d_k_at_cr100pct.png" % stretch_geometry)
